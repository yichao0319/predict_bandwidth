...
my $target_mean = 700000;  ## Bps
my $target_var = 25000;
my $target_len = 1001;  ## seconds
...
$file = "dummy.trace.txt";
print $file."\n" if($DEBUG0);

$target = $PREDICT_THROUGHPUT;
$interval = 1;
$win_size = 10;
$method = $METHOD_EWMA;
$ewma_alpha = 0.9;    
...
#####
## main
my $sim_time = $target_len;

#####
## first generate fake processed tcpdump output 
my @tmp_tput = ();
open FH, "> $file_path/$file" or dir $!;
for(my $t = $interval; $t < $sim_time; $t += $interval) {
    my $this_rcv = $target_mean - $target_var + int(rand(2*$target_var));
    push(@tmp_tput, $this_rcv);
    my $cur_var = variance(\@tmp_tput);
    print FH "$t $this_rcv $this_rcv $cur_var\n";
}
close FH;


#####
## then use the same code to generate output with specified format
my $err_sum = 0;
my $err_cnt = 0;
